...
Phase 10 closeout evidence lives at `docs/freestyle-viewer-platform/phase10/closeout.md`, with QA evidence in `docs/qa/phase10-production-telemetry-2026-04-24.md`. The current hard gate is `npm run check:phase10`; it includes the full local gate, measured fit/visual/memory/context quality gates, asset-budget evidence, Phase 9 cutover/rollback smokes, and operational browser smoke. `scripts/fit-quality-gate.mjs` intentionally fails without `--input`; use the matching `collect:*` command when adding new gate evidence.
...
- the current product preview path is still the CPU `@freestyle/fit-kernel` baseline for the fit-mesh data contract and performance gate. A Rust `packages/fit-kernel` crate plus `wasm-xpbd` adapter ABI now exists, but do not label the browser runtime as shipping WASM cloth until the worker bootstrap loads that artifact in product code
- `NEXT_PUBLIC_EXPERIMENTAL_XPBD_PREVIEW=1` may route the compatibility worker through `cpu-xpbd`; that path now applies the transferable fit-mesh buffer to visible garment `BufferGeometry` through normalized transfer binding. It is still a CPU/proxy baseline, not Rust/WASM or certification-grade cloth.
