...
- 공개/앱 셸 네비게이션은 좌측 사이드바가 아니라 상단 고정바를 기준으로 유지한다. 핵심 메뉴는 `홈 / 옷장 / 캔버스 / 발견 / 마이페이지`만 노출한다.
- Studio: 에셋 업로드/URL 임포트/장바구니 링크 임포트, 배경 제거, 캔버스 조합
  ...
  - Studio/Closet shared domain contract의 canonical source는 `packages/contracts`이며, 웹은 타입 전용으로 `@freestyle/contracts/domain-types` subpath를 사용한다
...
- `/app/closet`은 기본 진입부터 `v18 shell 기반의 좌측 glass status/customize panel + 중앙 3D human rig stage + 우측 fixed 3-column asset browser` 구조로 열리며, 문서 스크롤 없이 한 화면 안에서 동작하도록 고정한다
  - `/app/closet/item/[id]`는 선택한 에셋만 바로 여는 fitting lab entry point로 동작한다
  - 옷장/캔버스 3D dress-up은 공개 CC0 avatar/mannequin/jacket asset을 함께 사용하며, 출처는 `docs/OPEN_ASSET_CREDITS.md`에 기록한다
- 최신 `/app/closet` stage는 `TopControls / LeftPanel / MeasurementModal / AssetPanel / SceneSurface`로 분리된 `v18` overlay shell을 사용한다. 상의 / 외투 / 하의 / 신발은 분리 슬롯으로 교체되며, 마네킹/포즈 브라우저와 modal customizer가 같은 fitting 흐름 안에서 이어지고, 포즈/성별/체형 변경은 base body height 기준 스케일 경로로 유지한다
  - closet API가 비어 있거나 느려도 `apps/web/public/assets/closet/models/*`의 local starter pack으로 fitting flow가 끊기지 않도록 fallback한다
  - fitting 편집값은 `PATCH /v1/assets/:id`로 `assets.metadata.measurements`/`fitProfile`에 저장된다
  - canonical `BodyProfile`은 `{ simple, detailed? }` envelope를 사용하고, Studio/Closet은 legacy flat localStorage payload도 읽어서 새 구조로 정규화한다
  - 현재 렌더러는 `asset metadata + body profile` 기반의 실시간 preview이며, 실제 패턴/원단 물성 기반 cloth simulation은 아직 구현 범위 밖이다
  - 선택 레이어 1개 대상으로 preview-grade cloth 경로를 feature flag로 실험할 수 있으며, 비가용/오류 시 legacy preview로 즉시 fallback한다
  - 4A spike 판정/evidence는 `docs/CLOTH_SPIKE_4A.md`, `docs/qa/cloth-4a-2026-04-07.md`에 기록되어 있다
...
packages/contracts  # widget/api 공용 계약(zod schema + types)
...
- `NEXT_PUBLIC_CLOTH_MVP_ENABLED` (`true`면 선택 레이어 1개 대상 preview-grade cloth 경로 활성화 시도)
- `NEXT_PUBLIC_CLOTH_SPIKE_PASSED` (`true`일 때만 cloth integration 경로를 실제로 사용, 그 외에는 기존 경로 유지)
...
- 카나리 게이트: `docs/rollout-governance/canary-gates.md`
- 기능 플래그 매트릭스: `docs/rollout-governance/feature-flag-matrix.md`
- 포스트모템 템플릿: `infra/runbooks/postmortem-template.md`
- 서브에이전트 팀/프롬프트: `docs/SUBAGENT_TEAM.md`
- 기술 동향 점검 로그: `docs/TECH_WATCH.md`
- 리뉴얼 PRD: `docs/RENEWAL_PRD.md`
- 리뉴얼 정보구조/와이어프레임: `docs/RENEWAL_INFORMATION_ARCHITECTURE.md`
- 리뉴얼 앱 아키텍처: `docs/RENEWAL_APP_ARCHITECTURE.md`
- 아키텍처: `docs/architecture.md`
- API 계약: `docs/api-contract.md`
- Cloth Spike 기록: `docs/CLOTH_SPIKE_4A.md`
- Cloth Spike QA evidence: `docs/qa/cloth-4a-2026-04-07.md`
- Cloth worker 설계 초안: `docs/CLOTH_SIMULATE_JOB_DRAFT.md`
...
## 운영 메모
- Phase `0.5/4/5` 프로덕션 롤아웃은 `docs/rollout-governance/canary-gates.md`의 고정 progression(`1% -> 5% -> 25% -> 100%`)과 stop/rollback gate를 기준으로 진행한다.
- 배포 시작 전에는 `docs/rollout-governance/baseline-metrics-template.md`를 채우고, 소유권/승인 체계는 `docs/rollout-governance/ownership-manifest.md`를 따른다.
- UI 단순화를 위해 현재 `looks`, `decide`, `journal`, `examples`, `how-it-works`, `trends`, 레거시 `/profile`은 직접 노출하지 않고 남는 핵심 surface로 redirect한다.
- 모든 공개/앱 페이지는 홈 상단바와 동일한 고정 topbar를 사용한다. 메뉴는 `옷장 / 캔버스 / 발견 / 마이페이지`만 유지하고, 언어 토글과 로그인 버튼을 같은 위치에 둔다.
- 홈 hero 비주얼의 orbit는 개별 카드 배치가 아니라 이어붙인 이미지 band를 원형으로 회전시키는 방식으로 유지한다.
...
- 신규 API 계약:
  ...
  - `GET /v1/widget/config?tenant_id=...&product_id=...`
