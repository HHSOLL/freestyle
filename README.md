...
`Phase D` now also has a baseline offline HQ-fit path: `POST /v1/lab/jobs/fit-simulations` queues `fit_simulate_hq_v1`, the runtime worker processes it asynchronously, and the current artifact outputs are a typed `fit_map_json` plus `preview_png`. This is deliberate plumbing, not a claim that full draped-cloth output is already solved.
