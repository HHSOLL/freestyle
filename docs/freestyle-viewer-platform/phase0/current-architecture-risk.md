...
| No product-route latency instrumentation for first avatar paint or garment swap | `docs/freestyle-viewer-platform/phase0/baseline-performance.md` can only freeze command, asset, and seam-level evidence | Playwright visual regression and ops closeout smoke exist | no browser timer for cached preview, uncached preview, or avatar first-paint | `P0` |
