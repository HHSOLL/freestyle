...
- 3D Fitting: Studio에서 현재 캔버스 조합 또는 옷장 asset을 실시간 3D 마네킹에 입혀보고, 체형 치수와 의류 측정치를 함께 조절해 fit/ease를 확인
  - `/app/closet`은 기본 진입부터 `좌측 에셋 레일 + 중앙 3D 마네킹 스테이지 + 우측 조절 패널` 구조의 워크스페이스로 열린다
  ...
  - `/app/closet` 하단에는 Quaternius `Ultimate Modular Women Pack` 기반의 `Character Studio`가 함께 노출되어, 게임식 슬롯 교체형 공개 wardrobe preview도 바로 테스트할 수 있다
