...
- `Closet` now also has a direct HQ fit consumer seam:
  - users can request an HQ run for the currently equipped published garment
  - the left rail can poll the persisted record, render `preview_png`, and expose ordered artifact links plus `fitMapSummary`
  - this closes the product-consumer plumbing without overstating that live stage swap-in or solver-grade cloth are already solved
