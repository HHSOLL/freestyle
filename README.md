...
`Phase 8` of the viewer-platform refactor is now closed for the current repo-scoped baseline: HQ fit simulations persist a typed `artifact-lineage.json` sidecar, the lineage is inspectable through owner-scoped lab and admin-scoped read-only seams, and product state still keeps that lineage separate from the main fit-simulation detail payload. `Phase 8.5 / Batch 1` starts the admin tooling track with a dedicated HQ artifact inspection panel in `apps/admin`, but still stops short of approve/reject/certify workflow.
