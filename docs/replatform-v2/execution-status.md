...
| `Phase 4` 카나리 롤아웃 | `in_progress` | Web Vitals/JS error/widget flag foundation + deterministic canary audience wiring + add_to_cart 측정선 연결 완료. baseline 수치 확정/승인만 남음 |
...
- `Phase 4`: baseline 수치 확정, `1% -> 5% -> 25% -> 100%` 운영값 승인, canary 단계별 운영 로그 템플릿 확정
