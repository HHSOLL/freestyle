...
function GarmentLayerMesh({
  ...
  const baseTexture = useTexture(layer.textureUrl);
      ...
      <group>
        ...
        {[-0.22, 0.22].map((x) => (
          <mesh key={x} position={[x, -1.4, 0]} castShadow>
            <cylinderGeometry
              args={[layer.limbWidth ?? 0.32, (layer.limbWidth ?? 0.32) * 0.82, layer.limbLength ?? 1.5, 18]}
            />
            <meshStandardMaterial color={color} transparent opacity={shellOpacity} roughness={0.86} />
          </mesh>
        ))}
        <mesh position={[0, layer.shellYOffset + 0.05, frontOffset]}>
          ...
          <meshStandardMaterial map={texture} transparent alphaTest={0.08} side={THREE.DoubleSide} />
        </mesh>
      </group>
    );
  }
  ...
  return (
    <group>
      ...
      {[-1, 1].map((dir) => (
        <mesh
          key={dir}
          position={[dir * (layer.shellWidth * 0.42), 0.88, 0]}
          rotation={[0, 0, dir * 0.06]}
          castShadow
        >
          <cylinderGeometry
            args={[layer.limbWidth ?? 0.28, (layer.limbWidth ?? 0.28) * 0.86, layer.limbLength ?? 1.4, 18]}
          />
          <meshStandardMaterial color={color} transparent opacity={shellOpacity} roughness={0.84} />
        </mesh>
      ))}
...
export function MannequinScene3D({ body, layers, selectedAssetId }: MannequinScene3DProps) {
  ...
  return (
    <Canvas shadows camera={{ position: [0, 0.4, 6], fov: 26 }} className="h-full w-full" dpr={[1, 1.75]}>
