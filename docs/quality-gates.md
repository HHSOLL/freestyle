...
- `Reference-quality follow-up`: `NEXT_PUBLIC_EXPERIMENTAL_XPBD_PREVIEW=1` now exercises the same-origin worker `cpu-xpbd` path and emits transferable fit-mesh buffer metadata. This remains preview evidence only; HQ `draped_glb` lineage stays authored-merge until a separate solver-output exporter lands
