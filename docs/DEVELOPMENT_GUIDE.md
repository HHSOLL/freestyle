...
`/app/closet` now defaults to the `viewer-react -> viewer-core` product path through `apps/web/src/lib/closet-viewer-phase9.ts`. Use `NEXT_PUBLIC_CLOSET_VIEWER_PHASE9_KILL_SWITCH=true` or `NEXT_PUBLIC_VIEWER_HOST=runtime-3d` only for explicit rollback to the compatibility host. Keep that helper hydration-stable by reading the explicit `NEXT_PUBLIC_*` keys directly instead of handing `process.env` through wholesale.
...
Phase 9 starts at `docs/freestyle-viewer-platform/phase9/batch1.md`: the default `/app/closet` `viewer-react` path must keep route-level latency attrs plus preview-runtime / preview-engine evidence parity before it can be treated as the blocking UX latency gate, and the blocking smoke should wait for the outer stage-loading fallback to clear before reading latency attrs.
...
Reference-quality foundation work must keep solver and runtime claims precise: `packages/viewer-core/src/fitting/*` owns display/fit deformation transfer and footwear gates, `packages/viewer-core/src/resource-registry.ts` owns ref-count/LRU resource lifecycle primitives, and `tools/fit-quality/*` owns fixture-backed hard gates until hardware GPU and live runtime telemetry are connected.
