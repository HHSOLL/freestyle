...
`/app/closet` now owns the first explicit product cutover seam through `apps/web/src/lib/closet-viewer-phase9.ts`. The default product host is `viewer-react -> viewer-core`; use `NEXT_PUBLIC_CLOSET_VIEWER_PHASE9_KILL_SWITCH=true` or `NEXT_PUBLIC_VIEWER_HOST=runtime-3d` only for rollback/compatibility evidence. Keep that helper hydration-stable by reading the explicit `NEXT_PUBLIC_*` keys directly instead of handing `process.env` through wholesale.
...
Phase 9 starts at `docs/freestyle-viewer-platform/phase9/batch1.md`: the `/app/closet` `viewer-react` path must keep route-level latency attrs plus preview-runtime / preview-engine evidence parity before it can be treated as the blocking UX latency gate, and the blocking smoke should wait for the outer stage-loading fallback to clear before reading latency attrs.
Phase 9 rollback evidence widens in `docs/freestyle-viewer-platform/phase9/batch2.md`, and the current repo-scoped closeout lives at `docs/freestyle-viewer-platform/phase9/closeout.md`. Keep the default `viewer-react` path, kill switch, and rollback smoke aligned together.
Phase 10 closeout evidence lives at `docs/freestyle-viewer-platform/phase10/closeout.md`, with QA evidence in `docs/qa/phase10-production-telemetry-2026-04-24.md`. The current hard gate is `npm run check:phase10`; it includes the full local gate, `npm run test:fit-kernel:wasm`, measured fit/visual/memory/context quality gates, asset-budget evidence, Phase 9 cutover/rollback smokes, and operational browser smoke. `scripts/fit-quality-gate.mjs` intentionally fails without `--input`; use the matching `collect:*` command when adding new gate evidence.
...
Admin asset-generation intake is accepted at `/v1/admin/asset-generation`. It may register `DRAFT` or `TECH_CANDIDATE` requests from AI/vendor/partner/internal sources, but it must never auto-certify or auto-publish generated assets; production serving still requires the asset certification gate.
...
- keep `PREVIEW_DEFORMATION` as the shared transfer seam. Transform-only secondary motion must keep `transferMode: "secondary-motion-transform"`; XPBD fit-mesh solves must use `transferMode: "fit-mesh-deformation-buffer"` plus buffer metadata and the shared `@freestyle/fit-kernel` display-transfer helper, not React state or display-mesh solver input
- the current production default preview path is still the CPU `@freestyle/fit-kernel` baseline for the fit-mesh data contract and performance gate. The browser worker also supports the explicit Rust/WASM path with `NEXT_PUBLIC_EXPERIMENTAL_WASM_XPBD_PREVIEW=1`; that path lazy-loads `apps/web/public/workers/fit-kernel-wasm/freestyle_fit_kernel.js` and `_bg.wasm`.
- `NEXT_PUBLIC_EXPERIMENTAL_XPBD_PREVIEW=1` may route the compatibility worker through `cpu-xpbd`; that path applies the transferable fit-mesh buffer to visible garment `BufferGeometry` through normalized transfer binding. `NEXT_PUBLIC_EXPERIMENTAL_WASM_XPBD_PREVIEW=1` selects `wasm-preview`; if artifact loading fails, the worker falls back to `cpu-xpbd` and reports CPU execution truthfully.
- regenerate the committed browser artifact with `npm run build:fit-kernel:wasm` after changing Rust solver code. This requires Rust with the `wasm32-unknown-unknown` target and `wasm-bindgen-cli 0.2.118`.
- `npm run test:visual-golden:hardware` requires a self-hosted GitHub Actions runner labeled `self-hosted`, `linux`, `x64`, `hardware-gpu` and repository variable `RUN_HARDWARE_GPU_CI=true`; the collector reads `output/fit-quality/hardware-gpu-probe.latest.json` and does not trust a manual environment flag.
...
- keep that baseline honest: it currently persists `draped_glb`, typed `fit_map_json`, generated `preview_png`, and `metrics_json`. When the worker can apply deterministic fit-refiner vertex deformation to the starter garment GLB it records `drapeSource: "solver-output"` consistently across artifact metadata, metrics, and lineage; otherwise it falls back to `authored-scene-merge`.
