...
export function ClosetItemFittingLab({ assetId }: ClosetItemFittingLabProps) {
  ...
  const [activeAssetIds, setActiveAssetIds] = useState<string[]>([assetId]);
  ...
  const loadLab = useCallback(async () => {
    ...
    try {
      const [{ response: currentResponse, data: currentData }, { response: listResponse, data: listData }] =
        await Promise.all([
          apiFetchJson<unknown>(`/v1/assets/${assetId}`),
      ...
      setActiveAssetIds((prev) => {
        ...
        return [currentAsset.id, ...preserved];
      });
    } catch (nextError) {
  ...
  const selectedAsset = useMemo(
    () => activeAssets.find((asset) => asset.id === selectedAssetId) ?? currentAsset ?? activeAssets[0] ?? null,
    [activeAssets, currentAsset, selectedAssetId]
  );
  ...
  const handleSaveAssetMetadata = () => {
    ...
    startSaving(async () => {
      ...
      if (!response.ok) {
        setSaveError(getApiErrorMessage(data, copy.loadError));
        return;
      }
  ...
  if (loading) {
    return (
      <AppPageFrame eyebrow={copy.eyebrow} title={copy.title} description={copy.description}>
  ...
  return (
    <AppPageFrame
      ...
      title={language === 'ko' ? `${currentAsset.name} 피팅 랩` : `${currentAsset.name} Fitting Lab`}
      description={copy.description}
    >
      <section className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
        <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_220px]">
          <article className="overflow-hidden border border-black/8 bg-white">
            <div className="grid gap-5 p-5 md:grid-cols-[140px_minmax(0,1fr)]">
              <div className="aspect-[4/5] overflow-hidden rounded-[24px] bg-[#f3ece3]">
                <img src={currentAsset.imageSrc} alt={currentAsset.name} className="h-full w-full object-contain p-4" />
              </div>
              <div className="space-y-4">
                <div>
                  <p className="text-[11px] uppercase tracking-[0.18em] text-black/36">{copy.selectedGarment}</p>
                  <h2 className="mt-2 text-3xl font-semibold tracking-[-0.04em] text-black">{currentAsset.name}</h2>
                  <p className="mt-2 text-sm text-black/56">
                    {getClosetCategoryLabel(currentAsset.category, language)}
                    {currentAsset.brand ? ` · ${currentAsset.brand}` : ''}
                    {` · ${getWardrobeSourceLabel(currentAsset.source, language)}`}
                  </p>
                </div>
                <div className="flex flex-wrap gap-2 text-[11px] uppercase tracking-[0.18em] text-black/44">
                  <span className="rounded-full bg-black/[0.04] px-3 py-2">{copy.live}</span>
                  <span className="rounded-full bg-black/[0.04] px-3 py-2">
                    {currentAsset.metadata?.cutout?.removedBackground ? copy.cutoutReady : copy.cutoutFallback}
                  </span>
                  {currentAsset.metadata?.garmentProfile ? (
                    <span className="rounded-full bg-black/[0.04] px-3 py-2">Garment profile v1</span>
                  ) : null}
                </div>
                <div className="flex flex-wrap gap-3">
                  ...
                  {currentAsset.sourceUrl ? (
                    <Button asChild variant="outline" className="rounded-full">
                      <a href={currentAsset.sourceUrl} target="_blank" rel="noreferrer">
                        {copy.sourceLink}
          ...
          <article className="grid gap-3 border border-black/8 bg-[#111111] p-5 text-white">
            <div>
              <p className="text-[11px] uppercase tracking-[0.18em] text-white/42">{copy.source}</p>
              <p className="mt-2 text-sm leading-6 text-white/72">
                {currentAsset.sourceUrl ?? currentAsset.metadata?.sourceTitle ?? 'N/A'}
              </p>
            </div>
            <div>
              <p className="text-[11px] uppercase tracking-[0.18em] text-white/42">{copy.cutout}</p>
              <p className="mt-2 text-sm text-white/72">
                {currentAsset.metadata?.cutout?.removedBackground ? copy.cutoutReady : copy.cutoutFallback}
              </p>
            </div>
            <div>
              <p className="text-[11px] uppercase tracking-[0.18em] text-white/42">{copy.measurements}</p>
              <div className="mt-2 space-y-1 text-sm text-white/72">
                {selectedMeasurementSummary.length > 0 ? (
                  selectedMeasurementSummary.map((line) => <p key={line}>{line}</p>)
                ) : (
                  <p>{copy.missingMeasurements}</p>
                )}
              </div>
            </div>
          </article>
        ...
        <article className="border border-black/8 bg-[#f4ede3] p-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold tracking-[-0.04em] text-black">{copy.body}</h2>
      ...
      <section className="grid gap-4 xl:grid-cols-[320px_minmax(0,1fr)_360px]">
        <aside className="space-y-4 border border-black/8 bg-white p-5">
          ...
          <div className="space-y-2">
            {filteredAssets.length > 0 ? (
              ...
              <div className="rounded-[20px] border border-dashed border-black/15 px-4 py-6 text-sm text-black/50">
                {copy.empty}
              </div>
            )}
          </div>
        ...
        <section className="relative min-h-[720px] overflow-hidden border border-black/8 bg-[radial-gradient(circle_at_top,_rgba(255,255,255,0.92),_rgba(244,235,223,0.72)_45%,_rgba(239,228,214,0.92))]">
        ...
        <aside className="space-y-4 border border-black/8 bg-[#f4ede3] p-5">
          {selectedAsset ? (
            <>
              <div className="flex items-start gap-3">
                <div className="h-16 w-16 overflow-hidden rounded-[22px] bg-white shadow-sm">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-sm text-black/54">{getClosetCategoryLabel(selectedAsset.category, language)}</p>
                </div>
              </div>

              <section className="rounded-[24px] bg-white/80 p-4 shadow-sm">
            ...
            <div className="rounded-[24px] border border-dashed border-black/15 px-5 py-10 text-sm leading-7 text-black/56">
              {copy.noFitAsset}
            </div>
          )}
        </aside>
