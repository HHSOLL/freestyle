...
- a Phase 9 `Closet` cutover seam where `viewer-react -> viewer-core` is now the default product host, with `runtime-3d` retained only as an explicit kill-switch / global override fallback
- the default `viewer-react` `Closet` path avoids importing the `runtime-3d` compatibility preload path and exposes static-fit preview-runtime / preview-engine evidence attrs alongside the existing latency attrs
- the current Phase 9 baseline is closed for `/app/closet`: CI exercises the default `viewer-react` cutover path and the explicit `runtime-3d` rollback path through the kill switch
...
- a post-Phase 10 reference-quality foundation that adds `viewer-core` deformation transfer helpers, footwear hard gates, resource ref-count/LRU tracking, AI 3D provider intake seams, and fixture-backed fit / visual / preview / memory / context gate scripts
