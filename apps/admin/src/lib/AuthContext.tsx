...
import { setAdminApiAccessToken } from "@/lib/adminApi";
  ...
  useEffect(() => {
    ...
    supabase.auth
      ...
      .then(({ data }) => {
        ...
        setAdminApiAccessToken(data.session?.access_token ?? null);
        setIsLoading(false);
      })
      .catch(() => {
        if (!mounted) return;
    ...
    const {
      ...
      setAdminApiAccessToken(nextSession?.access_token ?? null);
      setIsLoading(false);
    });
