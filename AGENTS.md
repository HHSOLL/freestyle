...
6. Git 브랜치 워크플로우 강제
- read-only 작업이 아니면 항상 `main`에서 새 작업 브랜치를 만든 뒤 시작한다. 브랜치명은 기본적으로 `codex/<task-slug>` 형식을 따른다.
- 모든 변경은 작업 브랜치에서만 수행한다. `main`에서 직접 수정하거나 커밋하지 않는다.
- 작업 완료 후에는 최소 검증을 통과시키고, 작업 브랜치를 원격에 push한다.
- push 후에는 반드시 `main` 대상 PR을 생성하고, 검토/체크에 문제없으면 PR을 merge한다.
- merge가 끝나면 로컬을 `main`으로 되돌리고 원격 `main`을 pull 또는 fast-forward로 동기화한다.
- merge된 작업 브랜치는 로컬과 원격에서 모두 삭제하여, 작업 종료 시점에는 `main`만 남도록 유지한다.
- 사용자가 명시적으로 git/remote 작업을 금지한 경우에만 이 절차를 생략할 수 있다.
