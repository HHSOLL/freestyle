...
- a Phase 9 `Closet` cutover seam where `apps/web` now defaults product traffic to `viewer-react -> viewer-core`, while preserving a route-scoped kill switch back to `runtime-3d`
- the default `viewer-react` `Closet` path now warms runtime assets through the same preload delegation as the compatibility host and exposes static-fit preview-runtime / preview-engine evidence attrs alongside the existing latency attrs
- the current Phase 9 baseline is now closed for `/app/closet`: CI exercises the default `viewer-react` path and the `runtime-3d` rollback path through the kill switch
- a Phase 10 hard gate through `npm run check:phase10`, which layers fit/visual/memory/context quality gates, fit-kernel WASM artifact verification, asset-budget evidence, Phase 9 cutover/rollback smokes, and operational browser smoke on top of the full local gate
...
- `npm run collect:fit-golden`
- `npm run test:fit-golden`
- `npm run collect:visual-golden`
- `npm run test:visual-golden`
- `npm run test:fit-kernel:wasm`
- `npm run test:visual-golden:hardware` on a configured self-hosted Linux x64 GPU runner
- `npm run collect:memory-leak`
- `npm run test:memory-leak`
- `npm run collect:context-loss`
...
- measured fit golden, visual golden, memory-growth, and context-loss hard gate reports under `output/fit-quality/`
- operational browser smoke

The fit-quality gate fails closed if `--input` is omitted. The `test:*` hard-gate scripts collect `output/fit-quality/*.measured.json` first and then evaluate those measured inputs.
