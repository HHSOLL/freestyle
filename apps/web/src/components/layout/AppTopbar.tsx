...
import { appMeta } from '@/features/renewal-app/content';
...
export function AppTopbar() {
  ...
  const meta = appMeta[pathname as keyof typeof appMeta] ?? appMeta['/app'];
    ...
    <header className="sticky top-0 z-30 border-b border-black/8 bg-[rgba(255,255,255,0.82)] backdrop-blur-xl">
      <div className="flex items-center justify-between gap-4 px-5 py-4 sm:px-8">
        <div className="min-w-0">
          <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-black/38">Wardrobe OS</p>
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          {user ? (
            ...
            <Button asChild className="rounded-full bg-black px-4 text-white hover:bg-black/90">
              <Link href="/app/profile">Sign in</Link>
            </Button>
          )}
        </div>
      </div>
