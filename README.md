...
- Worker: Railway Worker (`workers/importer`, `workers/background_removal`, `workers/asset_processor`, `workers/evaluator`, `workers/tryon`)
...
npm run dev:all
```
`dev:all`은 `apps/web + apps/api + importer/background_removal/asset_processor/evaluator/tryon worker`를 함께 실행합니다.
